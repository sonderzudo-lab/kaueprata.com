(() => {
  ...
  const buildMobilePulse = () => {
    ...
    let ratio = 1;
    let onScreen = true;
    const startedAt = performance.now() - 3300;
    ...
    const drawSignal = (progress, strength) => {
      ...
      for (let index = tailLength; index >= 0; index -= 1) {
        const position = progress - index * 0.014;
        const x = (-0.14 + position * 1.28) * width;
